function C = mgaDesignV2(planetStart,rDepart,C3,planetTarget, ...
                         targetOrbitShape,maxN,optimalTrajectory,X)
% mgaDesignV2 - Genetic algorithm fitness function to determine the total
% Delta-V requirement of a given trajectory
%
% Inputs**
...
% optimalTrajectories = matrix contaniing information about previously
%                       identified trajectories by the genetic algoritm
...
%
% This function is called by ga in mgaGeneticCall.m to compute the
% fitness/cost of a randomize trajectory as provided by the algorithm.
%
...
gaPlanets = X(3:2+maxN); % gravity assist order
TOFs = X((maxN+3):(maxN+numGA+2)); % times of flight for each leg
tf = X(2*maxN+3); % time for the final leg of the trajectory
ang = X((2*maxN+4):(2*maxN+5)); % angle of fly-by during resonance

% When running the code in parallel, each agent needs access to the SPICE
% kernals
...
for i = 1:length(planetVisited)-1
    ...
    % Otherwise set the revolutions to 0
    else
        revs = 0;
    end
    ...
    % Otherwise, perform the following...
        ...
        if i == 1
            ...
            % % If the current leg is resonant, save empty arrays to be 
            % % computed later.
            % lambertSolutions{i,2} = [];
            % lambertSolutions{i+1,1} = [];
        end
...
for i = 1:length(lambertSolutions)-1
    ...
    % If the second position of this row is empty, then there is a
    % resonant orbit in the next leg. Perform the following
...
% If there are trajectories in optimalTrajectory, perform the following...
if ~isempty(optimalTrajectory)
    % For each item in optimalTrajectories, perform the following...
    for i = 1:length(optimalTrajectory(:,1))
        % Collect the number of gravity assists for the current saved
        % trajectory
        numFlyBys = optimalTrajectory(i,3);

        % If this equals the number of gravity assists used by the
        % trajectory being analyzed, then check to see if the planets and
        % their order are equal
        if numFlyBys == numGA
            if all(gaPlanets(1:numGA) == ...
                    optimalTrajectory(i,4:3+numFlyBys)) && ...
                    C > optimalTrajectory(i,1)
                % If they are and the cost function of this analysis is
                % worse than what is saved, then provide the current a
                % trajectory a bad score to prevent it from being
                % identified again
                C = C + C/optimalTrajectory(i,1)*1000;
            end
        end
    end
end

% Once finished, clear the kernals loaded for this script
cspice_kclear;
end
