function C = mgaTOFDesignV2(planetStart,rDepart,C3,planetTarget, ...
                targetOrbitShape,maxN,numGA,gaPlanets,optimalTrajectory,X)
% mgaTOFDesignV2 - Genetic algorithm fitness function to determine the 
% total Delta-V requirement of a given trajectory. A modification of
% mgaDesignV2.m to focus on the optimiztion of the times of flight
%
% Inputs**
...
% X(1) = departure date (julian days)
% X(2:(maxN+1)) = times of flight between gravity assists (days)
% X(maxN+2) = final leg time of flight (days)
% X(maxN+3) = angle 1 for possible resonant orbit (radians)
% X(maxN+4) = angle 2 for possible resonant orbit (radians)
%
% Outputs**
...
t0 = X(1); % launch time
TOFs = X(2:(numGA+1)); % times of flight for each leg
tf = X(maxN+2); % time for the final leg of the trajectory
ang = X((maxN+3):(maxN+4)); % angle of fly-by during resonance
...
for i = 1:length(planetVisited)-1
    ...
    if planetVisited(i) == planetVisited(i+1)
        ...
        for j = satRotations
            ...
            if revs ~= 0 %revs == 1 + beta || revs == j + beta
                break
            end
        end               
